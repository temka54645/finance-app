...
import { adminLoginAction, adminGoogleSigninAction } from "./actions";
...
export default function AdminLoginPage() {
  ...
  const onGoogleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    e.stopPropagation();
    setGoogleError(null);
    const email = googleEmail.trim();
    if (!email) {
      setGoogleError("Имэйл шаардлагатай");
      return;
    }
    startGoogleTransition(async () => {
      const fd = new FormData();
      fd.set("email", email);
      const res = await adminGoogleSigninAction(fd);
      if (!res.ok) {
        setGoogleError(res.error);
        return;
      }
      // Server-side зөвшөөрсөн — OAuth flow эхлүүлнэ. login_hint нь
      // signIn-ийн 3 дахь параметрт (authorizationParams) орно.
      await signIn(
        "google",
        { callbackUrl: "/sys/control" },
        { login_hint: res.email, prompt: "select_account" }
      );
    });
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-950 text-slate-100 px-4 py-10">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-7 shadow-2xl space-y-5">
          {/* Credentials login */}
          ...
          {/* Google login: тусдаа form */}
          <form onSubmit={onGoogleSubmit} className="space-y-3">
            <div className="space-y-1.5">
              <label className="block text-xs font-medium uppercase tracking-wide text-slate-400">
                Админ имэйл (Google)
              </label>
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-700 bg-slate-800 py-2.5 text-sm font-semibold text-slate-200 transition-all hover:bg-slate-700 disabled:opacity-60"
            >
              {googlePending ? (
